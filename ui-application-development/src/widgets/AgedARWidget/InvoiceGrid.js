...
const InvoiceGrid = (props) => {
  ...
  const billedAR = selectedProject.column.userProvidedColDef.field.split("ar_")[1];
  const billedARlabel =  selectedProject.column.userProvidedColDef.headerNameKey;
  let [arFirst, arSecond] = [0,0];
  if (billedAR == 'over_180'){
    arFirst = 180
  }else {
    [arFirst, arSecond] = split(billedAR.split()[0], 2);
  }
  ...
  const filteredData = (data) => {  
    ...
    { 
      ...
      // let date1; 
      // date1 = el.due_date?new Date(el.due_date.substr(0,10)):new Date(todayDate);
  ...
  // useEffect(() => {
  //   if(invoiceGridConfig.uiConfig && invoiceGridConfig.uiConfig.gridApi) {
